...
`
* To add the routing for all tabs, we can add the href values with paths defined in our routing, but that is not the prefered way as that reloads the whole app. We should use `routerLink='/servers' //AbsolutePath vs routerLink ='servers' //RelativePath   or [routerLink]="['/users','shoppingHistory']"`
* 
### Observables ###
